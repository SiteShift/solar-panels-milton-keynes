...
- [ ] One canonical per page, absolute URLs
- [ ] Robots.txt allows crawling; XML sitemap includes all indexable routes
- [ ] 200 status for indexable pages; 404/410 for gone content; 301 for moved
- [ ] Page titles 45–60 chars, unique; meta descriptions 140–160 chars, unique
- [ ] H1 1× per page, descriptive and keyphrase‑aligned
- [ ] Clean URLs with locality in slugs where relevant (e.g., `/solar-panels/milton-keynes/costs`)
- [ ] Core Web Vitals targets (mobile): LCP ≤ 2.5s, INP ≤ 200ms, CLS ≤ 0.1
- [ ] Images compressed, responsive sizes, width/height set; lazy load below the fold
- [ ] Preload hero image if LCP; avoid layout shifts
- [ ] No duplicate content (canonicalize, consolidate thin pages)
- [ ] 404 page includes helpful links/CTA back to quote

## 2) Information Architecture & Internal Linking
- [ ] Homepage anchors core intents and routes to all clusters
- [ ] Top navigation exposes: About, Services, Installers, Blog/Guides, Grants, Calculator, Quote
- [ ] Contextual internal links with descriptive anchor text in every article
- [ ] Section “Related reading” on all articles: 4–6 links across the cluster
- [ ] Breadcrumbs on all non‑home pages with JSON‑LD `BreadcrumbList`
- [ ] Footer deep links to key hub/cluster pages
- [ ] Siloing: keep MK‑specific solar topics tightly interlinked; avoid orphan pages
- [ ] Link priority: from hub → subpages → case studies → FAQs; always link back up to hub

## 3) Topical Map (Milton Keynes Solar Panels) — Hubs, Clusters, and Support

### 3.1 Core Hub Pages (commercial intent)
- [ ] Solar Panels Milton Keynes (Home/Hero) — primary money page, targets: “solar panels Milton Keynes”, “solar panel installers Milton Keynes”
- [ ] Services (Overview) — links to installation, battery storage, maintenance, EV charging
- [ ] Installers (Overview) — explains MCS, vetting, process; links to local installer profiles (if used)
- [ ] Pricing & Costs in Milton Keynes — detailed price ranges by system size, grants, payback
- [ ] Calculator — savings, payback, SEG earnings
- [ ] Grants 2025 — ECO4, 0% VAT, SEG; eligibility flow
- [ ] Contact / Quote — primary form page; reiterate trust, reviews, warranties

### 3.2 Commercial Support Pages (bottom/mid‑funnel)
- [ ] Solar Panel Installation Process (Milton Keynes)
- [ ] Solar Battery Storage (Benefits, brands, sizing)
- [ ] Inverters (String vs Micro, brands)
- [ ] Warranties & Guarantees (Panel, inverter, workmanship; 25‑year specifics)
- [ ] Maintenance & Monitoring (What’s included, annual checks)
- [ ] Roof Types in MK (Pitched, flat, slate, tile, new build)
- [ ] Planning Permission in Milton Keynes (Permitted development edge cases)
- [ ] Smart Export Guarantee in MK (Best providers, rates, setup)
- [ ] Finance Options (If applicable; otherwise guidance on ROI)

### 3.3 Informational Cluster Pages (top/mid‑funnel)
- [ ] Cost Breakdown (equipment vs labour; system sizes: 3kW/4kW/6kW/8kW)
- [ ] Payback & ROI for MK homes (bill sizes, usage patterns)
- [ ] Orientation, Shading & Tilt in MK (optimisation)
- [ ] Panel Types (Mono vs Poly, efficiency, degradation)
...
- [ ] Structured `FAQPage` schema per page where FAQs are the main content

## 4) On‑Page SEO for Each Page Type
- [ ] Title contains primary intent + locality (MK); no keyword stuffing
- [ ] H1 matches intent; H2s cover sub‑intents/PAA
- [ ] Opening paragraph answers the query directly (helpful content signal)
- [ ] TL;DR summary box for AI engines and skimmers
- [ ] Pros/cons or comparison tables where helpful
- [ ] Rich media: original photos, diagrams, iconography; descriptive alt text
- [ ] Internal links to: hub, sibling pages, relevant guides, case studies, quote
- [ ] CTA placement: above fold, mid‑page, end; consistent microcopy
- [ ] Last updated date for freshness; author byline and author box
...
- [ ] `ImageObject` for key images (logo, OG)

## 6) EEAT & Trust Signals
- [ ] Author system: each article shows author box with credentials, photo, links
- [ ] About page: team, certifications (MCS, RECC, NICEIC), mission, photos
...
- [ ] At least 10 high‑quality photos uploaded to GBP (work, team, projects)
- [ ] Weekly GBP posts: highlights, case studies, offers
- [ ] Q&A seeded with real common questions + answers
- [ ] Review acquisition flow: post‑install email/SMS with direct link; respond to all reviews
- [ ] Local citations on UK directories (Yell, 192, FreeIndex) and industry (MCS directory)
- [ ] Embed GBP map (if address is public) or service area mention

## 8) Conversion Optimisation (Content‑led)
- [ ] Primary CTAs: “Get Free Quote” consistent across pages; anchors to form
- [ ] Secondary CTAs: “Calculate Savings”, “Check Grant Eligibility”
- [ ] Benefit‑oriented copy near CTAs: savings, warranties, speed, MCS certified
- [ ] Multi‑step form (later): progress cues, low friction; for now single step is fine
- [ ] Trust proof near form: review rating, installs count, warranty
- [ ] Privacy reassurance near form: “We never share your details.” + link to Privacy
- [ ] Sticky/mobile footer CTA or top bar CTA
- [ ] Exit‑intent or bottom‑of‑page prompt (lightweight, non‑intrusive)
- [ ] Thank‑you page with next steps; encourage reply/booking
...
- [ ] H1 targets “Solar Panels Milton Keynes” with trust statement
- [ ] 40–60 word direct answer box (why choose us, key benefits)
- [ ] CTAs: Get Quote (primary), Calculate Savings (secondary)
- [ ] Trust signals: MCS/RECC/NICEIC, installs count, reviews, warranties
- [ ] Internal links: Costs, Grants, Installers, Process, Case Studies, Calculator, FAQ
- [ ] FAQ block for top 4–6 queries; `FAQPage` JSON‑LD limited to visible Q&A
- [ ] LocalBusiness/Organization/Service JSON‑LD id‑linked
- [ ] OG/Twitter image with local branding; compelling meta description
